  ...
  % invoice lines
      $for(service)$ @ \noalign{\vskip 2mm} \refstepcounter{pos} \thepos 
        ...
        $if(service.details)$\newline \begin{itemize} 
          ...
          $endfor$ \end{itemize}
        $endif$%
        & \dEUR[kind=symbol]{:={$service.price$}}\\
      $endfor$%
      \noalign{\vskip 2mm} \hline
  % subtotal if vat applies
  $if(from.vat-rate)$
    @ & @ \multicolumn{1}{r}{$lang.subtotal-text$:}                & \dEUR[kind=symbol]{:={sum(c1:[0,-1])}} \\ \hhline{~~-}
    @ & @ \multicolumn{1}{r}{$lang.vat-text$ $from.vat-rate$\%:}   & \dEUR[kind=symbol]{:={$from.vat-rate$/100*[0,-1]}} \\ \hhline{~~-}
  $endif$
  % total
  @ & @ \multicolumn{1}{r}{\textbf{$lang.total-text$:}}   & \dEUR[kind=symbol]{:={$if(from.vat-rate)$[0,-1]+[0,-2]$else$sum(c1:[0,-1])$endif$}} \\ \hhline{~~-}
\end{spreadtab}
