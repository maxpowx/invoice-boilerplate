...
\begin{spreadtab}{{tabular}[t t t]{lp{8.2cm}r}}
  ...
  & @ \textbf{$lang.unit-price-column-text$} \\ \hline
  % invoice lines
      $for(service)$
  ...
  $if(from.vat-rate)$
    % sub-total
    @ \multicolumn{2}{r}{$lang.subtotal-text$:}
    & \dCurrency{$invoice.currency$}{:={sum(c1:[0,-1])}} \\ \hhline{~~-}
    % vat rate
    @ \multicolumn{2}{r}{$lang.vat-text$ $from.vat-rate$\%:}
    & \dCurrency{$invoice.currency$}{:={$from.vat-rate$/100*[0,-1]}} \\ \hhline{~~-}
  $endif$
  % total
  @ \multicolumn{2}{r}{\textbf{$lang.total-text$:}}
  & \dCurrency{$invoice.currency$}{:={$if(from.vat-rate)$[0,-1]+[0,-2]$else$sum(c1:[0,-1])$endif$}} \\ \hhline{~~-}
\end{spreadtab}
